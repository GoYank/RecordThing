 ...
 * Describe : TODO
 **/
public class LoadMonthAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private List<RecordDataBean.DataBean.EventBean.FieldsBeanX> monthList;
    ...
    public LoadMonthAdapter(Context mContext, List<RecordDataBean.DataBean.EventBean.FieldsBeanX> monthList) {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder viewHolder, int position) {
        if (viewHolder instanceof RecyclerViewHolder) {
            ...
            Logger.e("tag", "monthposition=" + monthList.get(position).getMonth());
            recyclerViewHolder.tv_month.setText(monthList.get(position).getMonth()+"月");
            List<RecordDataBean.DataBean.EventBean.FieldsBeanX.FieldsBean> contentList = new ArrayList<>();
            for (int i=0;i<monthList.get(position).getFields().size();i++){
                contentList.add(monthList.get(position).getFields().get(i));
            }
            LoadMoreDataAdapter loadMoreDataAdapter = new LoadMoreDataAdapter(context,contentList);
            recyclerViewHolder.recy_content.setLayoutManager(new LinearLayoutManager(context));
            recyclerViewHolder.recy_content.setAdapter(loadMoreDataAdapter);
