 ...
 * Describe : TODO
 **/
public class LoadYearAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private List<RecordDataBean.DataBean.EventBean> list;
    private List<RecordDataBean.DataBean.EventBean.FieldsBeanX> monthList = new ArrayList<>();
    Context mcontext;
    //普通布局
    ...
    public LoadYearAdapter(Context mcontext, List<RecordDataBean.DataBean.EventBean> mList) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int position) {
        ...
        if (position == TYPE_ITEM) {
            View view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.item_year, viewGroup, false);
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder viewHolder, int position) {
        if (viewHolder instanceof RecyclerViewHolder) {
            ...
            recyclerViewHolder.tv_year.setText(list.get(position).getYear() + "年");
            for (int i = 0; i < list.get(position).getFields().size(); i++) {
                monthList.add(list.get(position).getFields().get(i));
            }
            LoadMonthAdapter loadMonthAdapter = new LoadMonthAdapter(mcontext, monthList);
            recyclerViewHolder.recy_month.setLayoutManager(new LinearLayoutManager(mcontext));
            recyclerViewHolder.recy_month.setAdapter(loadMonthAdapter);
    ...
    private class RecyclerViewHolder extends RecyclerView.ViewHolder {
        TextView tv_year;
        RecyclerView recy_month;

        RecyclerViewHolder(View itemView) {
            super(itemView);
            tv_year = itemView.findViewById(R.id.tv_year);
            recy_month = itemView.findViewById(R.id.recy_month);
        }
    }
